...
let selCat='food',editId=null,editDate=null,editType=null,addMode='expense',qIcon='',qLabel='',pendingCalDate=null;
...
function openQuickAdd(i,l){qIcon=i;qLabel=l;document.getElementById('quick-icon').textContent=i;document.getElementById('quick-label').textContent=l;document.getElementById('quick-amount').value='';document.getElementById('quick-note').value='';const t=tk();const qd=document.getElementById('quick-date');if(qd){qd.value=t;qd.max=t;}show('quick-modal');setTimeout(()=>document.getElementById('quick-amount').focus(),300)}
function closeQuickModal(){hide('quick-modal')}
function confirmQuickAdd(){const a=parseInt(document.getElementById('quick-amount').value);if(!a||a<=0){toast('⚠️ Nhập số tiền!');return}const c=CATS.find(x=>x.icon===qIcon)?.id||'other';const n=document.getElementById('quick-note').value.trim();const dk=(document.getElementById('quick-date')?.value)||tk();sv(dk,gid(),{category:c,amount:a,note:n||qLabel,createdAt:Date.now()},'expenses');closeQuickModal();snd();if(dk===tk())renderToday();updateHero();const pcd=pendingCalDate;if(pcd){renderCal();showCalDet(pcd);}const label=dk!==tk()?` (${dk.split('-').reverse().slice(0,2).join('/')})`:'' ;toast(`💸 ${qLabel}: −${fm(a)}${label}`)}
